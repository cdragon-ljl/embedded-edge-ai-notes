# 基于RV1126的端侧AI开发 · 第3期 模型转换之参数、算子约束与报错规避
...
你已经能转换一个 Resnet 了。但真实项目里的模型五花八门：有 YOLO、有 LPRNet、有 Transformer 的轻量变体，转换时经常遇到"报错、转出来不对、性能莫名变慢"。
...
运行时预处理 = `(x - mean) / std`，必须与模型训练时一致。一代 API 的格式是**嵌套列表**：
...
必须填写，它决定编译器生成哪一代 NPU 指令。填错（比如填 `'rk3568'`）会直接报错或生成跑不了的模型。
